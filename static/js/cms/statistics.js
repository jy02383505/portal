...
layui.use(['table','laydate','form','element'], function(){
        ...
        if(ip_list=='' || echart_list==''){
            ...
            for(var attack in ip_list){
                if(attack<10){
                    ...
                    '                        <span style="width:60%;overflow: hidden;white-space: nowrap;text-overflow: ellipsis;"><img class="flags_img" src="/static/image/flags/'+ip_list[attack].short_name.toLowerCase()+'_flag[1].gif" alt="" title="'+ip_list[attack].ip_address+'">'+ip_list[attack].ip_address+'</span>' +
    ...
    if(is_staff == 'False') {
        $('.daterangepicker:last-child').attr('id','daterangepicker')
        window.data_search = function () {       //拦截日志
            ...
            $.ajax({
                ...
                success: function (res) {
                    var startup_list;
                    if (res.status == true) {
                        if(parent_oblique[2]=='parent_sec_overview'){
                    ...
                    table.render({
                        ...
                        , cols: [[      //默认规则internation_trans.rule_id
                            {field: 'log_time', width: 160, title: gettext('拦截时间')}
                            , {field: 'atk_ip', width: 140, title: 'IP'}
                            , {field: 'target_url', width: "40%", event: 'url_event', title: 'URL'}
                            , {field: 'atk_type', width: 150, title: gettext('攻击类型')}
                            , {field: 'ruleid', width: 100, title: gettext('规则ID')}
                            , {
                                field: 'rule_status',
                                width: 100 ,
                                title: gettext('操作'),
        ...
        data_search();
        form.on('submit(log_search)',function(){
