...
(function (layui, window, factory) {
	...
	var v = '4.0.0.0910',
	    ...
	    quickBtns = [{ icon: 'xm-iconfont icon-quanxuan', name: gettext('全选'), click: function click(id, cm) {
			cm.selectAll(id, true, true);
		} }, { icon: 'xm-iconfont icon-qingkong', name: gettext('清空'), click: function click(id, cm) {
	...
	Common.prototype.changeShow = function (children, isShow) {
		...
		if (isShow) {
			...
			if(domain_list_length == choice_domain_dd){
				...
				$('.xm-select-label').show();
				$('.xm-hide-input,.xm-input').val('')
			}
